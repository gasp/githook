...
process.kill = function (pId, callback) {
  if (typeof process.list[pId] === 'undefined') {
    return callback(false);
  }

  var p = process.list[pId].process;
  p.kill();
  remove(p);
...
process.getCommand = function (pId) {
  if (typeof process.list[pId] === 'undefined') {
    return null;
  }
  return process.list[pId].command;
};
